...
class AbstractSolver(ABC):
    ...
    def report_results(self, iter_count, n_iters, loss_D_avg, loss_G_avg, fake_mnist_class_acc,
        ...
        if self.avg_losses_D[-1] < 0.005:
            ...
            self.save_testrun(success=False)
            sys.exit(1)
    ...
    def get_test_visuals(self, real_usps, fake_mnist, num_to_show=16):
        ...
        with torch.no_grad():
            ...
            gs = fig.add_gridspec(2, 2, width_ratios=[1, 5])
    ...
    def save_testrun(self, success):
        ...
        with open('testlog.csv', 'a') as f:
            writer = csv.writer(f)
            if success:
                result = self.classifier_accuracies[-1]
            else:
                result = "failed"
            row = [self.path, self.config.lambda_gan, self.config.lambda_cycle, self.config.lambda_gc,
                   self.config.lambda_reconst, self.config.lambda_dist, self.config.geometry, self.config.noise_var,
                   self.config.separate_G, result]
            writer.writerow(row)
